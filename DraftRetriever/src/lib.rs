...
impl Reader {
    ...
    fn search(
        ...
        for retrieved_token in &*results {
            if let Some(&first_token) = retrieved_token.first() {
                *cnt.entry(first_token).or_insert(0) += 1;
            }
        }
        ...
        for (&token, &count) in &cnt {
            // 使用元组排序：(-count, token) 实现：
            // 1. 主要按count降序（通过负数实现）
            // 2. 次要按token升序
            heap.push((-count, token));
        ...
        let mut token_counts = Vec::with_capacity(heap.len());
        while let Some((count, token)) = heap.pop() {
            token_counts.push((token, -count)); // 恢复实际count值
        }
        let total: i32 = token_counts.iter().map(|(_, c)| c).sum();

        let (tokens, frequencies): (Vec<i32>, Vec<f64>) = token_counts
        .into_iter()
